...
static int jl1xxx_dts_mdi_cfg_get(struct phy_device *phydev)
{
	...
	err = of_property_read_u16(of_node, "jl1xxx,mdi-enable",
				   &priv->mdi.enable);
	if (err < 0)
		return err;

	err = of_property_read_u16(of_node, "jl1xxx,mdi-rate",
				   &priv->mdi.rate);
	if (err < 0)
		return err;

	err = of_property_read_u16(of_node, "jl1xxx,mdi-amplitude",
				   &priv->mdi.amplitude);
	if (err < 0)
		return err;
...
static int jl1xxx_dts_rmii_cfg_get(struct phy_device *phydev)
{
	...
	err = of_property_read_u16(of_node, "jl1xxx,rmii-enable",
				   &priv->rmii.enable);
	if (err < 0)
		return err;

	err = of_property_read_u16(of_node, "jl1xxx,rmii-rx_timing",
				   &priv->rmii.rx_timing);
	if (err < 0)
		return err;

	err = of_property_read_u16(of_node, "jl1xxx,rmii-tx_timing",
				   &priv->rmii.tx_timing);
	if (err < 0)
		return err;
...
static int jl2xxx_dts_clk_cfg_get(struct phy_device *phydev)
{
	...
	err = of_property_read_u16(of_node, "jl2xxx,clk-enable",
				   &priv->clk.enable);
	if (err < 0)
		return err;
